...
function renderGalleryThumb(snapshotCanvas, index) {
  ...
  // add small per-thumbnail download button
  ...
  dl.addEventListener("click", () => {
    snapshotCanvas.toBlob((blob) => {
      ...
      a.href = url;
      a.download = `puzzlecam_photo_${Date.now()}.png`;
...
async function boot() {
  ...
  const watchdog = setTimeout(() => {
    if (!settled) showLoaderError("Loading is taking too long. Click retry or check your internet connection.");
  }, watchdogMs);
  try {
...
try {
  const seen = localStorage.getItem("puzzlecam_seen_tutorial");
  if (!seen && tutorialOverlay) {
    tutorialOverlay.classList.remove("hidden");
    setTimeout(() => { tutorialOverlay.classList.add("hidden"); localStorage.setItem("puzzlecam_seen_tutorial", "1"); }, 2200);
  }
} catch (e) {
}
